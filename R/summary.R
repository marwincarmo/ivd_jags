...
.summary_table <- function(stats, Kr ) {
  ...
  for (row_name in rownames(stats)) {
    ...
    } else if (grepl("^v\\_final\\[", row_name)) {
      rows_to_exclude <- c(rows_to_exclude,  row_name )
    }
  }
  ## Exclude rows
...
summary.ivd <- function(object, digits = 2, pip = 'all', ...) {
  ...
  if(length(zeta_index) != length(object$X_scale_names)) stop("Check zeta_index in summary.R")
  rownames(table)[zeta_index] <- paste("scl_", object$X_scale_names, sep = '')
  ## random effects sd
  ...
  patterns <- paste0(scale_indexes, "\\]") #paste0("\\[", scale_indexes, ",")
  replacements <- paste0(scale_ranef, "]") #paste0("[", scale_ranef, ",")
